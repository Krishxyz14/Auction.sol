...
contract Ebay{
  ...
  function createOffer(uint _auctionId) external payable auctionExists(_auctionId){
    ...
    require(msg.value>auction.min && msg.value>bestOffer.price,"msg.value is grater than the minimum and the best offer");
  ...
  function transaction(uint _auctionId) external auctionExists(_auctionId){
    ...
    auction.seller.transfer(bestOffer.price); //contract -> a ether transfer
